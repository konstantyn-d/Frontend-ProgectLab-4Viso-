...
function KPICard({ title, value, suffix, trend, trendLabel, sparkData, sparkColor, invertTrend }: KPICardProps) {
  ...
  const trendColor = isPositive ? 'text-[#10B981]' : isNegative ? 'text-[#E53E3E]' : 'text-[#6B6B6B]'
  const TrendIcon = trend === undefined ? null : trend > 0 ? TrendingUp : trend < 0 ? TrendingDown : Minus

  const chartData = sparkData.map((v, i) => ({ i, v }))

  return (
    <div className="bg-[#111111] border border-[#222222] p-5 hover:border-[#2E2E2E] transition-colors">
      <p className="text-[12px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-3">{title}</p>

      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="flex items-baseline gap-1">
            <span className="text-[32px] font-light text-[#F5F5F5] leading-none">{value}</span>
            {suffix && <span className="text-[14px] text-[#6B6B6B]">{suffix}</span>}
          </div>
          {trend !== undefined && TrendIcon && (
            <div className={cn('flex items-center gap-1 mt-2 text-[12px]', trendColor)}>
              <TrendIcon className="w-3 h-3" strokeWidth={1.5} />
              <span>{Math.abs(trend).toFixed(1)}%</span>
              <span className="text-[#3D3D3D]">{trendLabel}</span>
            </div>
          )}
          {trend === undefined && trendLabel && (
            <p className="text-[12px] text-[#3D3D3D] mt-2">{trendLabel}</p>
          )}
        </div>
